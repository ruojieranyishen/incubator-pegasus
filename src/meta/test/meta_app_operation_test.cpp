...
TEST_F(meta_app_operation_test, create_app)
{
    ...
    struct create_test
    {
        ...
        std::map<std::string,std::string> envs = {};
                 ...
                 {APP_NAME + "_11", 4, 3, 2, 3, 3, false, app_status::AS_INVALID, ERR_OK,
                    {{"rocksdb.num_levels", "5"}}},
                 {APP_NAME, 4, 3, 2, 3, 3, false, app_status::AS_INVALID, ERR_INVALID_PARAMETERS,
                    {{"rocksdb.write_buffer_size", "1000"}}},
                 {APP_NAME, 4, 3, 2, 3, 3, false, app_status::AS_INVALID, ERR_INVALID_PARAMETERS,
                    {{"rocksdb.write_buffer_size", "1073741824"}}},
                 {APP_NAME + "_12", 4, 3, 2, 3, 3, false, app_status::AS_INVALID, ERR_OK,
    ...
    for (auto test : tests) {
        ...
        auto err = create_app_test(
            test.partition_count, test.replica_count, test.success_if_exist, test.app_name, test.envs);
